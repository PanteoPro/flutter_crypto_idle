...
class _MarketBuyButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isActive = value > 0 && money > 0;
    return Padding(
...
class _MarketBuySumTokenWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final money = context.select((GameViewModel vm) => vm.state.money);
    return Text(
      (value * money / price.cost).toStringAsFixed(8),
...
class _MarketBuySumDollarWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final money = context.select((GameViewModel vm) => vm.state.money);
    return Text(
      S.of(context).text_with_dollar((value * money).toStringAsFixed(2)),
      style: AppFonts.body.copyWith(color: AppColors.white),
    );
  }
}
...
class _MarketSellButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isActive = value > 0 && (count ?? 0) > 0;
    return Padding(
      padding: const EdgeInsets.only(left: 12),
      child: GameButtonWidget.buy(
        text: 'ПРОДАТЬ',
...
class _MarketSellSumTokenWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final count = context.select((GameMarketCryptoViewModel vm) => vm.state.token?.count);
    return Text(
      (value * (count ?? 0)).toStringAsFixed(8),
      textAlign: TextAlign.right,
      style: AppFonts.body.copyWith(color: AppColors.white),
    );
  }
...
class _MarketSellSumDollarWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text(
      S.of(context).text_with_dollar((value * (count ?? 0) * price.cost).toStringAsFixed(2)),
      style: AppFonts.body.copyWith(color: AppColors.white),
    );
  }
}
